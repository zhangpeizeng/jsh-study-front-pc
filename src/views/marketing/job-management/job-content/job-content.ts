...
export default class jobContent extends Vue {
  ...
  query(): void {
    this.loading = true;
    this.getTableList(this.studyTerminal);
  }
  ...
  //获取列表
  getTableList(studyTerminal: any): void {
    this.shake = true;
    this.loading = true;
    this.form.studyTerminalCode = studyTerminal;
    Http.post(MarkeTing.listHomeworkSubmit, this.form)
      .then(res => {
        this.shake = false;
        if (res.data.success) {
