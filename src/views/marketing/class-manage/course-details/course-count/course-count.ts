...
import courseNostudy from "./not-learning/not-learning.vue";
import courseStatistical from "./course-quantity/course-quantity.vue";
import correction from "./components/correction/correction.vue";
import examComplete from "./exam-complete/exam-complete.vue";
import {
  Button,
  Form,
  FormItem,
...
export default class jobManagement extends Vue {
  ...
  isShowExam: boolean = false;
  isShowAssignment: boolean = false;
  ...
  onExportCustomerAlready(): void {
    ...
    const obj = this.form;
    Http.post(Exchange.classExportAradyLearn, obj)
      .then(resp => {
  ...
  checkDownloadStatus(taskId: string): void {
    let owner = this;
    Http.get(Exchange.classExportZhjLearn, { taskId: taskId })
      .then(resp => {
  ...
  getCustomerAlreadyTableList(): void {
    ...
    this.form.classId = this.idClass;
    this.form.gmCodeList = this.centerName;
    this.form.industryCodeList = this.productName;
    this.form.studyStatus = this.studyStatus;
    Http.post(MarkeTing.classDetailAlready, this.form)
      .then(res => {
        this.shake = false;
        console.log(res);
        if (res.data.success) {
  ...
  getSellerAlreadyTableList(): void {
    this.shake = true;
    this.loading = true;
    this.sellerForm.classId = this.idClass;
    Http.post(MarkeTing.classDetailAlready, this.sellerForm)
      .then(res => {
        this.shake = false;
        console.log(res);
        if (res.data.success) {
  ...
  getSellerNoStudyTableList(): void {
    this.loading = true;
    Http.post(MarkeTing.classDetailNostudy, this.sellerNoStudyForm)
      .then(res => {
        if (res.data.success) {
  ...
  private created(): void {
    ...
    this.terminalCode = this.studyTerminalCode; // 学习终端
    this.activeCode = this.studyTerminalCode; // 学习终端
