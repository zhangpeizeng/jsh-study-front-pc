...
export default class Login extends Vue {
  ...
  private censusList() {
    this.loading = true;
    Http.post(MarkeTing.alreadylistexaminfo, {
      gmCodeList: this.centerName, // 中心
      orgNameOrCode: this.organName, // 组织编码或者名称
      accountId: this.studentorname, // 学员id
      industryCodeList: this.productName,
  ...
  private onExportLearningDetailReport(): void {
    ...
    let owner = this;
    Http.post(Exchange.classExportExamReport, {
      gameCodeList: this.centerName, // 中心
      orgNameOrCode: this.organName, // 组织编码或者名称
      accountId: this.studentorname, // 学员id
      industryCodeList: this.productName,
      classId: this.idClass,
  ...
  checkDownloadStatus(taskId: string): void {
    let owner = this;
    Http.get(Exchange.classExportExamReportStatus, {
      taskId: taskId
    })
      .then(res => {
  ...
  downloadFile(taskId: string): void {
    let owner = this;
    Http.get(
      Exchange.classExportExamReportTask,
      {
        taskId: taskId
      },
      {
