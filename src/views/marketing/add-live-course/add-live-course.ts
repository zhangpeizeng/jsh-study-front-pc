...
export default class AddLiveCourse extends Vue {
    ...
    Http.get(MarkeTing.getCourseDetail, { id: this.id })
      .then(res => {
        ...
        if (data.success || data.success == null) {
          ...
          this.courseLiveType = this.detailObj.courseLiveType; // 直播方式
  ...
  private insertCourse(status: number) {
    ...
    if (!this.courseLiveType) {
      this.$message.error("请选择直播方式");
      return;
    }
    if (this.optionsNews.length == 0) {
    ...
    let params = {
      ...
      courseLiveType: this.courseLiveType, // 直播方式
