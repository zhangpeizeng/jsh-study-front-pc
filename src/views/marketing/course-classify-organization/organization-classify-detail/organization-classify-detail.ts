...
export default class organizationClassifyDetail extends Vue {
  tabsName: any = "1";
  courseName: any = "";
  tableData: any = [];
  detailObj: any = "";
  ...
  private update() {
    this.$router.push({
      ...
      query: {
        id: this.id,
        saveType: "1",
        firstCode: this.detailObj.parentOneId,
  ...
  // 查课程列表
  private getClassifyCourseOrgInfo() {
    Http.post(MarkeTing.getClassifyCourseOrgInfo, {
      id: this.id,
  ...
  private getclassifyDetail() {
    Http.get(MarkeTing.getclassifyOrgDetail, {
      id: this.id
    }).then(res => {
      const { data } = res;
      if (data.success) {
