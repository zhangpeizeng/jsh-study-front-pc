...
export default class jobManagement extends Vue {
  ...
  exportList(): void {
    ...
    Http.post(Exchange.unstudyArady, this.form)
      .then(resp => {
  ...
  checkDownloadStatus(taskId: string): void {
    let owner = this;
    Http.get(Exchange.unstudyZhiL, { taskId: taskId })
      .then(resp => {
  ...
  getTableList(): void {
    this.shake = true;
    this.loading = true;
    Http.post(MarkeTing.detailNostudy, this.form)
      .then(res => {
        this.shake = false;
        console.log(res);
        if (res.data.success) {
