...
import { BrandListTableData } from "@/views/marketing/shop-class/shop-class-detail/interface";
      ...
      .then(res => {
        ...
        if (res.data.success) {
          ...
          if (this.courseType === "1") {
            if (this.detail.productCode && this.detail.productCode !== "") {
              this.getgoods(this.detail.productCode);
            }
          }
          if (
    ...
    Http.get(MarkeTing.getdocurl, { etag: val })
      .then(res => {
        ...
        if (res.data.success) {
          ...
          // this.downVideo(res.data.data.url, 1, 2);
