...
export default class SelectTestPaper extends Vue {
  ...
  handleType(list: any) {
    list.forEach((item: any) => {
      if (item.attributeTypeCode === "type") {
        ...
        this.levelStr = item.certificateAttributeStr;
      }
    });
  }
  ...
  // 数据回显
  dataEcho(data: any) {
    if (data.form.certificateType === "1") {
      this.form = data.form;
      this.gradeList = data.gradeList;
      this.grade = data.gradeList;
      this.typeList = data.typeList;
      this.type = data.typeList;
      this.natureList = data.natureList;
      this.nature = data.natureList;
      this.isType = "1";
    }
    debugger;
    if (data.form.certificateType === "2") {
      this.form2 = data.form;
      this.isType = "2";
  ...
  private confirm() {
    if (this.isType === "1") {
      ...
      if (this.form2.certificateName) {
        ...
        this.form2.levelStr = this.levelStr;
      ...
      .then(res => {
        ...
        this.gradeList.forEach((item: any) => {
          this.grade.forEach((item2: any) => {
            if (item.id === item2) {
              data.levelStr
                ? (data.levelStr = data.levelStr + "、" + item.attributeName)
                : (data.levelStr = data.levelStr + item.attributeName);
            }
          });
        });
